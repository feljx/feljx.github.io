...
var copy_email = function copy_email(_) {
  // @ts-ignore
  email_input.select();
  document.execCommand('copy');
};

email.addEventListener('click', copy_email); // GSAP SKEW ANIMATION

gsapWithCSS.registerPlugin(ScrollTrigger);
var proxy = {
  skew: 0
},
    skewSetter = gsapWithCSS.quickSetter('.skew', 'skewY', 'deg'),
    src_clamp = gsapWithCSS.utils.clamp(-5, 5);
ScrollTrigger.create({
  onUpdate: function onUpdate(self) {
    var skew = src_clamp(self.getVelocity() / -300); // Only do something if the skew is MORE severe.
